...
leaflet() %>% 
  ...
  addCircleMarkers(phWI.1$dec_long_va,phWI.1$dec_lat_va,
                   color = "red", radius=3, stroke=FALSE,
                   fillOpacity = 0.8, opacity = 0.8,
                   popup=phWI.1$station_nm)
...
* Multiple agencies
    ...
    + EPA data comes from the STORET database (this includes many state and tribal organizations)
...
* In R, one vector (or column in a data frame) can only have __ONE__ timezone attribute
    ...
    + `dataRetrival` queries could easily span timezones    
