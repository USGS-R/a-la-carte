...
* upload the shapefile to [GDP](http://cida.usgs.gov/gdp/client/#!advanced/spatial)  
...
* host the feature somewhere as a WFS (web feature service):  
```{r, echo=T}
stencil <- webgeom(url='https://www.sciencebase.gov/catalogMaps/mapping/ows/54668b7ce4b04d4b7dbda52d')
query(stencil, 'geoms')
geom(stencil) <- "sb:managed_wiscoNoZ_wgs84"
attribute(stencil) <- "WBDY_NAME" # note this is NOT the unique identifier for features in this WFS ("WBDY_WBIC" is)
head(query(stencil, 'values'))
```
...
* upload the shapefile to [GDP](http://cida.usgs.gov/gdp/client/#!advanced/spatial):
 - upload zipped shapefile using the [uploader](http://cida.usgs.gov/gdp/client/#!advanced/spatial)
```{r, echo=T}
stencil <- webgeom()
query(stencil, 'geoms')
...
#job <- geoknife(stencil, 'daymet', wait = TRUE)
#head(result(job))
```
...
<p align="center">
  <a href="http://cida.usgs.gov/gdp/"><img src="../images/GDP_screen.png" alt="Geo Data Portal" align="center" width="90%"/></a>
</p>
